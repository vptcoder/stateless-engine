...
export class SimpleStateEngine {
	...
	validateResults: { current: number | string; action: string; result: boolean }[] | null;
	...
	validate<V>(current: number | string, action: string, validationInput: V) {
		...
		if (!transition) {
			throw new Error('No transition exists with this initial state and this action.');
		}
		...
		if (
			!transition.validationMethod ||
			transition.validationMethod(validationInput) === true
		) {
			this.validateResults?.push({ current: current, action: action, result: true });
		} else {
			this.validateResults?.push({ current: current, action: action, result: false });
